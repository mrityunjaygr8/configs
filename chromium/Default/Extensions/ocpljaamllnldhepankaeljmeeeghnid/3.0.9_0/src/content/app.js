...
// implemented by many other extensions that enhance Gmail and Salesforce (necessary to keep up with
// updates to those products).
//
//
//
//
...
var BUILD_TARGET = /(^|\.)(sales)?force\.com$/.test(window.location.hostname) ? 'salesforce' : 'gmail-inbox';
...
function loadCSS(environment, domain, numRetries, retryCount  ) {
  ...
  var cssPath = domain + '/src/styles-' + BUILD_TARGET + '.css';
...
function loadScript(environment, domain, numRetries, withCors, retryCount  ) {
  ...
  script.setAttribute('src', domain + '/src/build-' + BUILD_TARGET + '.js');
