  ...
  doubleClickTimer = setTimeout(function() {
    chrome.windows.get(tab.windowId, function(win) {
      chrome.storage.local.get(null  , function(storage) {
        ...
        if (!opened) refreshBrowser('gmail-inbox', true );
...
function refreshBrowser(target, bringToForeground) {
  if ((target !== 'gmail-inbox') && (target !== 'salesforce')) return;
  ...
  chrome.windows.getAll({
    ...
    windows.forEach(function(win) {
      win.tabs.forEach(function(tab) {
        ...
        if (target === 'gmail-inbox') {
          ...
          if (!/https?:\/\/(.*\.)?(sales)?force\.com/.test(tab.url)) return;
        } else {
    ...
    if (bringToForeground && !foundExisting) {
      chrome.tabs.create({
        ...
        url: (target === 'gmail-inbox') ? 'https://mail.google.com' : 'https://salesforce.com',
        active: true
      });
    }
  });
...
ExtensionMessageBus.on('refreshGmail', function(payload, sender, callback) {
  refreshBrowser('salesforce');
  refreshBrowser('gmail-inbox', payload  );
...
var selfProduction = 'https://d1j5o6e2vipffp.cloudfront.net';
var selfStaging = 'https://dzdsh0uck5hox.cloudfront.net';
var selfLocal = 'https://extension-local.mixmax.com';
...
var iframeHosts = [
  // For the analytics bridge.
  selfProduction,
  selfStaging,
  selfLocal,
  // Allow Mixmax for pricing upsells.
  'http://mixmax.com',
  'https://mixmax.com',
  'https://' + '*.mixmax.com',
  'http://' + '*.mixmax.com',
...
chrome.webRequest.onHeadersReceived.addListener(function(details) {
  for (var i = 0; i < details.responseHeaders.length; i++) {
    ...
    if (isCSPHeader) {
      ...
      csp = csp.replace('style-src', 'style-src ' + styleHosts);
      csp = csp.replace('frame-src', 'frame-src ' + iframeHosts);
      csp = csp.replace('connect-src', 'connect-src ' + connectHosts);
